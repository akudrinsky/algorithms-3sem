...
private:
    ...
    std::unordered_map<const Node*, char> colors;

    bool checkRec(const Node* cur, const std::vector<char>& alphabet);
...
bool AhoCorasik::Node::isTerminal() const
{
...
) 
{
    ...
    ON_DEBUG(
        photo(std::to_string(colors.size()) + ".png");
    )
    ...
    for (const char& c: alphabet) 
    {
        ...
        if (next->nearestTerminal == nullptr and !next->isTerminal() and checkRec(next, alphabet))
            return true;
    }
...
void AhoCorasik::photo(std::string pictName) {
    ...
    while (!verts.empty())
    {
        ...
        )
        {
            ...
            constexpr int colorLen = 10;
            char color[colorLen] = {};
            if (colors.count(child) == 0)
                strncpy(color, "grey", colorLen);
            else if (colors.at(child) == 't')
                strncpy(color, "green", colorLen);
            else 
                strncpy(color, "red", colorLen);

            if (child == automatonState)
            {
                #pragma GCC diagnostic ignored "-Wformat"
                fprintf (pFile, "\t%lu [shape=circle, style=\"filled\", color=%s, label=\"\"];\n", child, color);
            }
            else if (child->isTerminal())
            {
                #pragma GCC diagnostic ignored "-Wformat"
                fprintf (pFile, "\t%lu [shape=circle, style=\"filled\", color=%s, label=\"T\"];\n", child, color);
            }
            else
            {
                #pragma GCC diagnostic ignored "-Wformat"
